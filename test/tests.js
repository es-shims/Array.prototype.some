...
module.exports = function (some, t) {
	t.test('passes the correct values to the callback', function (st) {
		...
		var context = {};
		some(arr, function (value, key, list) {
	...
	t.test('does not visit elements added to the array after it has begun', function (st) {
		...
		st.equal(i, 3, 'some callback only called thrice');
	...
	t.test('empty array', function (st) {
		st.equal(some([], trueThunk), false, 'true thunk callback yields false');
		st.equal(some([], falseThunk), false, 'false thunk callback yields false');

		st.end();
	});

	t.equal(some([1, 2, 3], trueThunk), true, 'returns true if some callback returns true');
	t.equal(some([1, 2, 3], falseThunk), false, 'returns false if no callback returns true');

	t.test('stopping after N elements', function (st) {
		st.test('no context', function (sst) {
			var actual = {};
			var count = 0;
			some(getTestArr(), function (obj, index) {
				actual[index] = obj;
				count += 1;
				return count === 3;
			});
		...
		st.test('arraylike, no context', function (sst) {
			var actual = {};
			var count = 0;
			some(createArrayLikeFromArray(getTestArr()), function (obj, index) {
				actual[index] = obj;
				count += 1;
				return count === 3;
			});
